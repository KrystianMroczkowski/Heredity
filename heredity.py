...
def inheriting_gene_probability(genes_number):
    if genes_number == 0:
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if not mother or not father:
            ...
            gene_from_mother_prob = inheriting_gene_probability(mother_genes)
            gene_from_father_prob = inheriting_gene_probability(father_genes)

            if person_genes == 0:
                person_prob *= (1 - gene_from_father_prob) * (1 - gene_from_mother_prob)
            elif person_genes == 1:
                person_prob *= (gene_from_mother_prob * (1 - gene_from_father_prob) +
                                gene_from_father_prob * (1 - gene_from_mother_prob))
            else:
                person_prob *= gene_from_father_prob * gene_from_mother_prob
